    ...
    const check = (old, now) => {
      ...
      if (presentedFrames > baseline) {
        const processingDuration =
          ...
          0;
        const timediff = now - old; // diff in time between requesting the frame and actually when requestAnimatioNFrame fired AND frame has changed
        callback(now, {
          presentationTime: now + processingDuration * 1000, // The time at which the user agent submitted the frame for composition.
